  ...
  def self.stock_display_report(branch, brand)
    Jde.find_by_sql("
      ...
      (
        SELECT * FROM 
        (
          SELECT  MAX(ROWNUM) AS RNK , MAX(SDDELN) AS SDDELN, MAX(SDDOCO) AS SDDOCO, SDLOTN, SDSHAN, MAX(SDSRP1) AS SDSRP1, 
                MAX(SDUORG) AS SDUORG, MAX(SDDSC1) AS SDDSC1, MAX(SDDSC2) AS SDDSC2, MAX(SDLITM) AS SDLITM, MAX(SDDRQJ) AS SDDRQJ, 
                MAX(SDADDJ) AS SDADDJ, MAX(SDMCU) AS SDMCU 
                FROM PRODDTA.F4211 WHERE SDDCTO = 'ST' AND SDDELN > 1
                GROUP BY SDLOTN, SDSHAN ORDER BY RNK DESC
